...
parser.add_argument('--lcode', default='ko', help='ISO 639-1 code of target language. See `lcodes.txt`.')
...
def make_wordvectors():
    ...
    with codecs.open('./corpus.txt', 'r', 'utf-8') as fin:
        while 1:
            line = fin.readline()
            if not line: break
    ...
    model = gensim.models.Word2Vec(sents, size=vector_size, 
                                   min_count=min_count, 
    ...
    # Save to file
    with codecs.open('./{}.tsv'.format(lcode), 'w', 'utf-8') as fout:
